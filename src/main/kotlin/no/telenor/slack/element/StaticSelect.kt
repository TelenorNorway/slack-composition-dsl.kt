...
class StaticSelect(
	...
	fun option(value: String, text: String, description: String? = null, selected: Boolean = false) {
		...
		if (selected) {
			require(selectedOption == null) { "Static select can only have one selected option" }
	...
	class OptionProxy(private val self: StaticSelect) {
		fun option(value: String, text: String, description: String? = null, selected: Boolean = false) =
			self.option(value, text, description, selected)
	}

	override fun into() = BlockElements.staticSelect { select ->
